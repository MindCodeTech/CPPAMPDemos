...
bool Test1()
{
    ...
    array<int, 1> arr(size, results, accelerator(accelerator::cpu_accelerator).default_view);
...
bool Test2()
{
    ...
    try {
        parallel_for_each(accelerator(accelerator::cpu_accelerator).default_view, extent<1>(size), [&](index<1> idx) __GPU {
            arr[idx] += 1;
        });
