...
bool test_feature()
{
...

    {
		...
		parallel_for_each(e1, [&](index<_rank> idx) __GPU {
			src[idx] = static_cast<_type>(src.extent.size());
		});

		// rank and contents are matched
        if ((e1 != src.extent) || (e1.size() != src.extent.size()))
            return false;

        vector<_type> dst_data = src;
		return VerifyAllSameValue(dst_data, static_cast<_type>(src.extent.size())) == -1;
	}
}
