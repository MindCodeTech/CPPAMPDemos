...
Amp_menu::Amp_menu(const ini::App_menu_init& init, ldr::Loader& ldr, slv::Solver& slv)
...
void Amp_menu::init_menu(void)
{
	...
	solve_menu.set_option(unique_ptr<Option>(new Action("Set accelerator for TSP solver", slv_set_acc, &solve_menu)));
	solve_menu.set_option(unique_ptr<Option>(new Action("Set parameters for TSP solver", slv_set_prm, &solve_menu)));
