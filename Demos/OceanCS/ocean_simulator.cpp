...
float phillips(MHVECTOR2 K, MHVECTOR2 W, float v, float a, float dir_depend)
{
...
m_fft_plan(3, m_av)
{
    ...
    memset(h0_data.data(), 0, height_map_size * sizeof(MHVECTOR2));
...
void ocean_simulator::init_height_map(ocean_parameter& params, vector<float_2> &out_h0, vector<float> &out_omega)
{
	...
	//XMVECTOR vwind_dir = XMLoadFloat2(&params.wind_dir);
	wind_dir = MHVECTOR2::Normalize(params.wind_dir);
	//XMStoreFloat2(&wind_dir, XMVector2Normalize(vwind_dir));
...
void ocean_simulator::update_displacement_map(float time)
{
    ...
    UINT strides[1] = {sizeof(MHVECTOR4)};
    UINT offsets[1] = {0};
...
HRESULT CompileShaderFromFile( WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut )
{
    ...
    // open the file
    HANDLE hFile = CreateFile( str, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
...
{
    ID3D11Query *queryHandle = nullptr;
    ...
    HRESULT hr;
    do {
        hr = m_pd3dImmediateContext->GetData(reinterpret_cast<ID3D11Asynchronous*>(queryHandle), nullptr, 0, 0);
    } while (hr == S_FALSE);
...
void ocean_simulator::init_dx11(int hmap_dim, int output_size, UINT float2_stride)
{
    ...
    tstring filePath = GetFilePath::GetFilePath(_T("\\..\\Media\\Shaders\\ocean_simulator_vs_ps.hlsl"));
    ...
    vb_desc.ByteWidth = 4 * sizeof(MHVECTOR4);
