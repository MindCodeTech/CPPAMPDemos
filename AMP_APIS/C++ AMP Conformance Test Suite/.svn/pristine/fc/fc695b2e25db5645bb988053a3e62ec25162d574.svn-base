...
runall_result test1(accelerator_view &acc_view)
{
	...
	// On CPU acceleators
	if( acc_view.accelerator.device_path == accelerator::cpu_accelerator)
	{
		for( int i = 0 ; i < static_cast<int>(e.size()); i++)
		{
			if(src_arr[i] != i+1)
	...
	
	{
		...
		parallel_for_each(src_arr.extent,[&src_arr,dst_av](index<1> idx)restrict(amp){
...
runall_result test2(accelerator_view &acc_view)
{
	...
	
	{
		...
		result &= REPORT_RESULT(dst_av.extent == extent<1>(ext[1]));
		
		// GPU
		dst_av.discard_data();
		parallel_for_each(extent<1>(1),[=,&src_arr](index<1>) restrict(amp,cpu){
			array_view<int> results = src_arr[0];
			for(int i = 0;i < static_cast<int>(dst_av.extent.size()) ;i++)
				dst_av[i] = results[i];
		});
		copy(dst_av,actual_v.begin());
		result &= REPORT_RESULT(Verify(actual_v, expected_v));
...
runall_result test3(accelerator_view &acc_view)
{
	...
	
	{
		...
		copy(dst_av,actual_v.begin());
		result &= REPORT_RESULT(Verify(actual_v, expected_v));
		result &= REPORT_RESULT(dst_av.extent == extent<2>(ext[1],ext[2]));
		
		// GPU
		dst_av.discard_data();
		parallel_for_each(extent<1>(1),[=,&src_arr](index<1>) restrict(amp,cpu){
			array_view<int,2> results = src_arr[0];
			for(int i = 0;i < dst_av.extent[0] ;i++)
			{
				for(int j = 0; j < dst_av.extent[1];j++)
				{
...
{
    accelerator_view acc_view = require_device().default_view;
    
    runall_result result;
    accelerator_view cpu_view = accelerator(accelerator::cpu_accelerator).default_view;
