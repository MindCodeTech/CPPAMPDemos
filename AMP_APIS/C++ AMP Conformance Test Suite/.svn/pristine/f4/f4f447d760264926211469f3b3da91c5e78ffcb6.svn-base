...
runall_result test1(accelerator *device)
{
	...
	accelerator_view default_av = device->default_view;
	accelerator_view av = device->create_view();

	array_view<int,1> arrViewSrc(M);
	arrViewSrc.refresh();
	
	parallel_for_each(default_av,arrViewSrc.extent,[=](index<1> idx) restrict(amp){
		arrViewSrc(idx) = idx[0];
	});
	
	parallel_for_each(av,arrViewSrc.extent,[=](index<1> idx) restrict(amp){
		arrViewSrc(idx) += 10;
	});
	arrViewSrc.refresh();
	...
	parallel_for_each(arrViewSrc.extent,[=](index<1> idx) restrict(amp){
