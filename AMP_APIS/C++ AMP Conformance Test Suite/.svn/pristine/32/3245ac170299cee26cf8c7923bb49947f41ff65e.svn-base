...
int main()
{
    ...
    if(m != av.extent[0]) // Verify extent
    {
    ...
    if(n != av.extent[1]) // Verify extent
    {
    ...
    if(o != av.extent[2]) // Verify extent
    {
    ...
    parallel_for_each(acc_view, av.extent, [=] (index<3> idx) __GPU
    {
        av[idx]++;
    });
    ...
    for(int i = 0; i < av.extent[0]; i++)
    {
        for(int j = 0; j < av.extent[1]; j++)
        {
            ...
            {
                auto expected = vec_copy[i * av.extent[1] * av.extent[2] + j * av.extent[2] + k] + 1;
