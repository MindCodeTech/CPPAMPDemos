...
runall_result test_main()
{
    ...
    result &= REPORT_RESULT((av != acc.default_view) == true);
    result &= REPORT_RESULT(av.queuing_mode == queuing_mode_automatic);
    result &= REPORT_RESULT(is_accelerator_view_operable(av));

    av = acc.create_view(queuing_mode_immediate);
    result &= REPORT_RESULT(av.queuing_mode == queuing_mode_immediate);
    result &= REPORT_RESULT(is_accelerator_view_operable(av));
