...
void scan_tiled(array_view<_type> input, array_view<_type> output);

template <int _tile_size, typename _type>
void compute_tilewise_scan(array_view<_type> input,
                           array_view<_type> tilewise_scans,
                           array_view<_type> sums_of_tiles);


// Compute prefix of prefix
template <int _tile_size, typename _type>
void prefix_scan(array_view<_type> a) {
  array<_type> atemp(a.get_extent());
  scan_tiled<_tile_size>(array_view<_type>(a), array_view<_type>(atemp));
  copy(atemp, a);
}

template <int _tile_size, typename _type>
void scan_tiled(array_view<_type> input, array_view<_type> output) {
  int sz = input.get_extent()[0];
  ...
  compute_tilewise_scan<_tile_size>(array_view<_type>(input),
                                    array_view<_type>(output),
                                    array_view<_type>(scan_of_sums_of_tiles));
  ...
  if(number_of_tiles > 1) {
    ...
    if(sz > 0) {
      parallel_for_each(extent<1>(sz), 
          [=, &scan_of_sums_of_tiles](concurrency::index<1> idx) restrict(amp) {
...
void compute_tilewise_scan(array_view<_type> input,
  ...
  parallel_for_each(extent<1>(number_of_threads).tile<_tile_size>(),
...
void scan<_type, _tile_size>::execute() {
  array_view<_type, 1> a_values(values.size(), values);
...
int main() {
  ...
  _scan_int.execute();
  pass1 = _scan_int.verify();

  scan<float, TILE_SIZE> _scan_float(NUM_OF_ELEMENTS * 4);
  _scan_float.execute();
  pass2 = _scan_float.verify();

  if(default_device.get_supports_double_precision()) {
    scan<float, TILE_SIZE> _scan_double(NUM_OF_ELEMENTS * 2);

    _scan_double.execute();
    pass3 = _scan_double.verify();
  }
