...
runall_result test8(accelerator& device)
{
	...
	array<int, 2> arr(arr_v_c, device.default_view, ACCESS_TYPE);
...
{
	extent<2> arr_extent = CreateRandomExtent<2>(64);        
    array<float, 2> arr(arr_extent, device.default_view, ACCESS_TYPE);
...
{
	array<int, 1> arr1(64, device.default_view, ACCESS_TYPE);
	array<int, 2> arr2(64, 64, device.default_view, ACCESS_TYPE);
	array<int, 3> arr3(64, 64, 64, device.default_view, ACCESS_TYPE);
...
runall_result test11(accelerator& device)
{
	extent<3> arr_extent = CreateRandomExtent<3>(64);  
	std::vector<int> cont(arr_extent.size(), 10);
	array<int, 3> arr(arr_extent, cont.begin(), device.default_view, ACCESS_TYPE);
...
runall_result test12(accelerator& device)
{
	extent<2> arr_extent = CreateRandomExtent<2>(64);  
	std::vector<int> cont(arr_extent.size(), 10);
	array<int, 2> arr(arr_extent, cont.begin(), cont.end(), device.default_view, ACCESS_TYPE);
...
{
	std::vector<int> cont1(64, 10);
	array<int, 1> arr1(64, cont1.begin(), device.default_view, ACCESS_TYPE);
	
	std::vector<int> cont2(64 * 64, 10);
	array<int, 2> arr2(64, 64, cont2.begin(), device.default_view, ACCESS_TYPE);
	
	std::vector<int> cont3(64 * 64 * 64, 10);
	array<int, 3> arr3(64, 64, 64, cont3.begin(), device.default_view, ACCESS_TYPE);
...
{
	std::vector<int> cont1(64, 10);
	array<int, 1> arr1(64, cont1.begin(), cont1.end(), device.default_view, ACCESS_TYPE);
	
	std::vector<int> cont2(64 * 64, 10);
	array<int, 2> arr2(64, 64, cont2.begin(), cont2.end(), device.default_view, ACCESS_TYPE);
	
	std::vector<int> cont3(64 * 64 * 64, 10);
	array<int, 3> arr3(64, 64, 64, cont3.begin(), cont3.end(), device.default_view, ACCESS_TYPE);
...
{		
	...
	if(!device.supports_cpu_shared_memory)
	{
		WLog() << "The accelerator " << device.description << " does not support zero copy: Skipping" << std::endl;
		return runall_skip;
	}
