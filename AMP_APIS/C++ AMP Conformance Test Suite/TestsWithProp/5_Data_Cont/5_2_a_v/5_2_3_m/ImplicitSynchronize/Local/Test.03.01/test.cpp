...
runall_result test_main()
{
    ...
    std::vector<int> random_data(a.extent.size());
    Fill(random_data);
    array<int, 1> random_array(a.extent, random_data.begin(), random_data.end());
    
    // create a section verify that writes to the underlying data are visible
    parallel_for_each(av.extent, [=, &random_array](index<1> i) __GPU {
        array_view<int, 1> section = av.section(av.extent);
