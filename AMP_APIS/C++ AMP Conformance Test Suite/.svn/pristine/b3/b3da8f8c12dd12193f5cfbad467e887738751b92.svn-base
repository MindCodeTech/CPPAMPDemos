...
bool Test1(const accelerator& acc)
{
    ...
    array<unsigned int, 2> sA(e, accelerator(accelerator::cpu_accelerator).default_view, acc.default_view);
    array<unsigned int, 2> sB(e, accelerator(accelerator::cpu_accelerator).default_view, acc.default_view);
    ...
    array<unsigned int, 2> mA(e, acc.default_view);
    copy(sA, mA);

    array<unsigned int, 2> mB(e, acc.default_view);
    copy(sB, mB);

    array<unsigned int, 2> mC(e, acc.default_view);

    parallel_for_each(mC.extent, [&](index<2> idx) __GPU
    {
    ...
    array<unsigned int, 2> sC(e, accelerator(accelerator::cpu_accelerator).default_view, acc.default_view);
    copy(mC, sC);

    // Compute on CPU
    pA = sA.data();
    array<unsigned int, 2> ref(e, accelerator(accelerator::cpu_accelerator).default_view);
