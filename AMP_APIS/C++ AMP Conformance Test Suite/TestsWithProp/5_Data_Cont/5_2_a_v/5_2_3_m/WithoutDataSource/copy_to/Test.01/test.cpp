...
runall_result test1(const accelerator_view &av)
{
	...
	parallel_for_each(av, arrViewSrc.extent, [=](const index<2> &idx) restrict(amp) {
...
runall_result test4(const accelerator_view &av)
{
	...
	parallel_for_each(av,arrSrc.extent,[=,&arrSrc](index<2> idx) restrict(amp){
...
runall_result test5(const accelerator_view &av)
{
	...
	int comp_result = 0;
	array_view<int,1> av_compare_result(1,&comp_result);
	parallel_for_each(av,arrViewSrc.extent,[=](index<2> idx) restrict(amp){
