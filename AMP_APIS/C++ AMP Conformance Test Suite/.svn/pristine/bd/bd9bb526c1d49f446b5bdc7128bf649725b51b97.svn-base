...
{
    {
        ...
        if (src.extent.size() != _D0 * _D1 * _D2)
            return false;
    }
...
bool test_feature_itr(extent<_rank> _extent, _Iterator _first, _Iterator _last)
{
    ...
    parallel_for_each(src1.extent, [&] (index<_rank> idx) __GPU_ONLY
    {
        src1(idx) = src1[idx]*2;
    });
    ...
    parallel_for_each(src2.extent, [&] (index<_rank> idx) __GPU_ONLY
    {
    ...
    parallel_for_each(dst.extent, [&] (index<_rank> idx) __GPU_ONLY
    {
...
bool test_feature_itr(extent<_rank> _e, _BeginIterator _first)
{
    ...
    parallel_for_each(src.extent, [&] (index<_rank> idx) __GPU_ONLY
    {
        dst[idx] = src(idx);
    });
