...
bool test_feature(const std::vector<accelerator>& devices)
{
    for (size_t i = 0; i < devices.size()-1; i++)
    {
		...
		WLog() << "device1 = devices[" << i << "] = " << pdevice1.device_path << std::endl;
		WLog() << "device2 = devices[" << (i+1) << "] = " << pdevice2.device_path << std::endl;

        {
            if (!test_accl_staging_buffer_constructor<_type, _rank, accelerator_view>(pdevice1.default_view, pdevice2.default_view))
                return false;
        ...
        {
            if (!test_accl_staging_buffer_constructor<_type, _rank, accelerator_view>(pdevice2.default_view, pdevice1.default_view))
                return false;
