...
float reduction_simple_2(const std::vector<float>& source) {
  ...
  if((element_count % window_width) != 0 && element_count > window_width) {
    tail_sum = std::accumulate(source.begin() + 
    ((element_count - 1) / window_width) * window_width, source.end(), 0.f);
  }
  array_view<float, 1> av_tail_sum(1, &tail_sum);
...
float reduction_tiled_1(const std::vector<float>& source) {
  static_assert(IS_POWER_OF_2(_tile_size),
                "Tile size must be a positive integer power of two!");
  ...
  if (!check_tiled_precondition(_tile_size, element_count)) {
    std::cout << "Warning,reduction_tiled_1 "
              <<"is not designed for the current problem size." << std::endl;
  }
    ...
    parallel_for_each(extent<1>(element_count).tile<_tile_size>(), 
      ...
      for(unsigned s = 1; s < _tile_size; s *= 2) {
        if (local_idx % (2 * s) == 0) {
          ...
          tile_data[local_idx] = tile_data[local_idx] + tile_data[local_idx + s];
        }
        tidx.barrier.wait();
      }
...
float reduction_tiled_2(const std::vector<float>& source) {
  ...
  if (!check_tiled_precondition(_tile_size, element_count)) {
    std::cout << "Warning, reduction_tiled_2 "
              <<"is not designed for the current problem size." << std::endl;
  }
...
float reduction_tiled_4(const std::vector<float>& source) {
  ...
  while (element_count >= _tile_size && (element_count % (_tile_size * 2)) == 0)
  {
...
float reduction_cascade(const std::vector<float>& source)
{
  ...
  parallel_for_each(extent<1>(_tile_count * _tile_size).tile<_tile_size>(), 
                    [=, &a, &a_partial_result] (tiled_index<_tile_size> tidx) 
    restrict(amp) {
    // Use tile_static as a scratchpad memory.
    tile_static float tile_data[_tile_size];
...
int main() {
  ...
  for(const auto& func : functions) {
    ...
    if (fp_equal(result, expected_result, 0.05f)) {
      ...
      std::cout << "FAILED: " << func.second << " expected " << expected_result 
                << " but found " << result << "!" << std::endl;
    }
  }
  float result1 = reduction_tiled_1<tile_size>(source);
  if (fp_equal(result1, expected_result, 0.05f)) {
    std::cout << "SUCCESS: reduction_tiled_1." << std::endl;
  }
  else {
    std::cout << "FAILED: reduction_tiled_1" << " expected " << expected_result 
              << " but found " << result1 << "!" << std::endl;
  }

  float result2 = reduction_tiled_2<tile_size>(source);
  if (fp_equal(result2, expected_result, 0.05f)) {
    std::cout << "SUCCESS: reduction_tiled_2."<< std::endl;
  }
  else {
    std::cout << "FAILED: " << "result2" << " expected " << expected_result
              << " but found " << result2 << "!" << std::endl;
  }

  float result3 = reduction_tiled_3<tile_size>(source);
  if (fp_equal(result3, expected_result, 0.05f)) {
    std::cout << "SUCCESS: reduction_tiled_3." << std::endl;
  }
  else {
    std::cout << "FAILED: reduction_tiled_3" << " expected " << expected_result
              << " but found " << result3 << "!" << std::endl;
  }

  float result4 = reduction_tiled_4<tile_size>(source);
  if (fp_equal(result4, expected_result, 0.05f)) {
    std::cout << "SUCCESS: reduction_tiled_4." << std::endl;
  }
  else {
    std::cout << "FAILED: reduction_tiled_4" << " expected " << expected_result
              << " but found " << result4 << "!" << std::endl;
  }

  float result5 = reduction_cascade<tile_size, 128>(source);
  if (fp_equal(result5, expected_result, 0.05f)){
    std::cout << "SUCCESS: reduction_cascade." << std::endl;
  }
  else{
    std::cout << "FAILED: reduction_cascade" << " expected " << expected_result
              << " but found " << result5 << "!" << std::endl;
  }
}
