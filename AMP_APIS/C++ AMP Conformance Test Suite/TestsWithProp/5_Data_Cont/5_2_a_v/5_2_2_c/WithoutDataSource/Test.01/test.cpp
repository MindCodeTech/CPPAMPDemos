...
bool test(const accelerator_view &av)
{
	...
	result &= REPORT_RESULT(arrViewSum.extent == ext);
	parallel_for_each(av, arrViewSum.extent, [=](const index<2> &idx) restrict(amp) {
		arrViewSum[idx] = arrViewA[idx] + arrViewB[idx];
	});

	array_view<int, 2> arrViewDiff(ext);
	parallel_for_each(av, arrViewDiff.extent.tile<TILE_DIM0,TILE_DIM1>(), [=](const tiled_index<TILE_DIM0,TILE_DIM1> &idx) restrict(amp) {
...
bool test1(const accelerator_view &av)
{
	...
	result &= REPORT_RESULT(arrViewSum.extent == extent<1>(M));
	parallel_for_each(av, arrViewSum.extent, [=](const index<1> &idx) restrict(amp) {
		arrViewSum[idx] = arrViewA[idx] + arrViewB[idx];
	});

	array_view<int, 1> arrViewDiff(M);
	parallel_for_each(av, arrViewDiff.extent.tile<TILE_DIM0>(), [=](const tiled_index<TILE_DIM0> &idx) restrict(amp) {
		arrViewDiff[idx] = arrViewA[idx] - arrViewB[idx];
	});
...
bool test3(const accelerator_view &av)
{
	...
	result &= REPORT_RESULT(arrViewSum.extent == extent<3>(M,N,O));
	parallel_for_each(av, arrViewSum.extent, [=](const index<3> &idx) restrict(amp) {
		arrViewSum[idx] = arrViewA[idx] + arrViewB[idx];
	});

	array_view<int, 3> arrViewDiff(M, N, O);
	parallel_for_each(av, arrViewDiff.extent.tile<TILE_DIM0,TILE_DIM1,TILE_DIM2>(), [=](const tiled_index<TILE_DIM0,TILE_DIM1,TILE_DIM2> &idx) restrict(amp) {
		arrViewDiff[idx] = arrViewA[idx] - arrViewB[idx];
	});
