...
int test_feature()
{
    ...
    for (size_t i = 0; i < devices.size(); i++)
    {
		...
		printf("Device %d: %ws (%ws)\n", i, device.description.c_str(), device.device_path.c_str());
        ...
        test_accl_constructor<_type, _rank, accelerator_view>(device.default_view);
...
int test_feature()
{
    ...
    for (size_t i = 0; i < devices.size(); i++)
    {
        ...
        test_accl_constructor<double, _rank, accelerator_view>(device.default_view);
