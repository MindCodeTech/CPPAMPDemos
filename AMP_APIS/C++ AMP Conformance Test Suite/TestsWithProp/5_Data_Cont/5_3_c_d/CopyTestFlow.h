...
{
	if(gpu_acc.supports_cpu_shared_memory)
	{
		//Concurrency::Test::WLog() << "Accelerator " << gpu_acc.description << " supports zero copy" << std::endl;
...
{	
	if(!gpu_acc1.supports_cpu_shared_memory && !gpu_acc2.supports_cpu_shared_memory)
	{
		access_types_vec.push_back(std::make_tuple(concurrency::access_type_auto, concurrency::access_type_auto));
	}
	else if(gpu_acc1.supports_cpu_shared_memory && !gpu_acc2.supports_cpu_shared_memory)
	{
		//Concurrency::Test::WLog() << "Accelerator " << gpu_acc1.description << " supports zero copy" << std::endl;
	...
	{
		//Concurrency::Test::WLog() << "Accelerator " << gpu_acc2.description << " supports zero copy" << std::endl;
...
{
	index_iterator<_rank> idx_iter(amp_container.extent);
...
void ModifyOnAcceleratorView(const concurrency::accelerator_view& av, concurrency::array<_type, _rank>& arr, _type value)
{
	...
	if(av.accelerator.device_path == concurrency::accelerator::cpu_accelerator)
	{
		ModifyOnCpu<_type, _rank, array>(arr, value);
	}
	else
	{
		parallel_for_each(av, arr.extent, [&,value](index<_rank> idx) restrict(amp)
		{
			arr[idx] += value;
		});
	}
...
void ModifyOnAcceleratorView(const concurrency::accelerator_view& av, concurrency::array_view<_type, _rank>& arr_v, _type value)
{
	...
	{
		parallel_for_each(av, arr_v.extent, [=](index<_rank> idx) restrict(amp)
		{
			arr_v[idx] += value;
		});
	}
							  	...
							  	concurrency::access_type dest_access_type)
{
	...
	array<_type, _rank> target_arr(src_arr.extent, target_av, target_access_type);
	...
	array<_type, _rank> dest_arr(src_arr.extent, src_av, dest_access_type);
							  	...
							  	concurrency::access_type dest_access_type)
{
	...
	array<_type, _rank> target_data_arr(src_arr.extent, target_av, target_access_type);
							  	...
							  	concurrency::access_type dest_access_type)
{
	...
	array<_type, _rank> stg_arr(src_arr.extent, cpu_av, stg_arr_av);
	...
	array<_type, _rank> dest_arr(src_arr.extent, arr_av, dest_access_type);
...
{	
	...
	// Create source data
	std::vector<_type> data(non_contig_arr_v.extent.size(), static_cast<_type>(INIT_VALUE));
	array<_type, _rank> src_arr(non_contig_arr_v.extent, data.begin(), src_av, src_access_type);
...
{		
	...
	array<_type, _rank> target_arr(src_arr_v.extent, target_av, target_access_type);
	...
	array<_type, _rank> dest_data_arr(src_arr_v.extent, src_av, dest_access_type);
							  	...
							  	concurrency::access_type dest_access_type)
{
	...
	array<_type, _rank> target_data_arr(src_arr_v.extent, target_av, target_access_type);
	array_view<_type, _rank> target_arr_v(target_data_arr);
...
bool CopyAndVerifyFromArrayViewToIterator(const concurrency::accelerator_view& av,   concurrency::access_type src_access_type)
{
	...
	_stl_cont<_type> src_stl_cont(src_arr_v.extent.size(), static_cast<_type>(_rank));
							  	...
							  	concurrency::access_type dest_access_type)
{
	...
	array<_type, _rank> stg_arr(src_arr_v.extent, cpu_av, stg_arr_av);
	...
	array<_type, _rank> dest_data_arr(src_arr_v.extent, arr_v_av, dest_access_type);
	array_view<_type, _rank> dest_arr_v(dest_data_arr);
...
{		
	...
	// Create source data
	std::vector<_type> data(non_contig_arr_v.extent.size(), static_cast<_type>(INIT_VALUE));
	array<_type, _rank> src_data_arr(non_contig_arr_v.extent, data.begin(), src_av, target_access_type);
	array_view<_type, _rank> src_arr_v(src_data_arr);
	...
	array<_type, _rank> dest_data_arr(non_contig_arr_v.extent, src_av, dest_access_type);
	array_view<_type, _rank> dest_arr_v(dest_data_arr);
...
{		
	...
	array<_type, _rank> target_arr(src_non_contig_arr_v.extent, target_av, target_access_type);
							  	...
							  	concurrency::access_type target_access_type)
{
	...
	array<_type, _rank> target_data_arr(src_non_contig_arr_v.extent, target_av, target_access_type);
	array_view<_type, _rank> target_arr_v(target_data_arr);
...
bool CopyAndVerifyFromNonContigArrayViewToIterator(const concurrency::accelerator_view& av,   concurrency::access_type src_access_type)
{
	...
	_stl_cont<_type> target_stl_cont(src_non_contig_arr_v.extent.size(), static_cast<_type>(_rank));
							  	...
							  	concurrency::access_type src_access_type)
{
	...
	array<_type, _rank> target_stg_arr(src_non_contig_arr_v.extent, cpu_av, stg_arr_av);
...
{		
	...
	// Copy: non-contiguous array_view (src_av) -> staging array (stg_arr_v)
	array<_type, _rank> target_data_arr(src_non_contig_arr_v.extent, target_av, target_access_type);
	array_view<_type, _rank> target_non_contig_arr_v = target_data_arr.section(idx, src_non_contig_arr_v.extent);
...
bool CopyAndVerifyFromArrayViewConstToIterator(const concurrency::accelerator_view& av,   concurrency::access_type src_access_type)
{
	...
	_stl_cont<_type> target_stl_cont(src_arr_v.extent.size());
							  	...
							  	concurrency::access_type src_access_type)
{
	...
	array<_type, _rank> target_stg_arr(src_arr_v.extent, cpu_av, stg_arr_av);
...
bool CopyAndVerifyFromIteratorToNonContigArrayView(const concurrency::accelerator_view& target_av,   concurrency::access_type target_access_type)
{
	...
	// Create source data
	_stl_cont<_type> src_stl_cont(target_non_contig_arr_v.extent.size(), static_cast<_type>(_rank));
							  	...
							  	concurrency::access_type target_access_type)
{
	...
	array<_type, _rank> target_arr(src_stg_arr.extent, arr_av, target_access_type);
	...
	array<_type, _rank> dest_stg_arr(src_stg_arr.extent, cpu_av, stg_arr_av);
							  	...
							  	concurrency::access_type target_access_type)
{
	...
	array<_type, _rank> data_arr(src_stg_arr.extent, arr_v_av, target_access_type);
...
bool CopyAndVerifyFromStagingArrayToStagingArray(const concurrency::accelerator_view& src_cpu_av, const concurrency::accelerator_view& src_stg_arr_av, const concurrency::accelerator_view& target_cpu_av, const concurrency::accelerator_view& target_stg_arr_av)
{
	...
	array<_type, _rank> target_stg_arr(src_stg_arr.extent, target_cpu_av, target_stg_arr_av);
	...
	array<_type, _rank> dest_stg_arr(src_stg_arr.extent, src_cpu_av, src_stg_arr_av);
							  	...
							  	concurrency::access_type target_access_type)
{
	...
	// Create source data
	std::vector<_type> data(non_contig_arr_v.extent.size(), static_cast<_type>(INIT_VALUE));
	array<_type, _rank> src_stg_arr(non_contig_arr_v.extent, data.begin(), cpu_av, stg_arr_av);
