...
bool CopyBetweenArrayAndArray(Concurrency::accelerator& srcDevice, Concurrency::accelerator& destDevice)
{
	...
	array<Udt, 2> srcArray(arrayExtent, srcCont.begin(), srcDevice.default_view);
	Log() << "Created array on source device of " << srcArray.extent << std::endl;
	
	// Create destination array
	array<Udt, 2> destArray(arrayExtent, destDevice.default_view);
	Log() << "Created array on destination device of " << destArray.extent  << std::endl;
...
bool CopyBetweenArrayViewAndArrayView(Concurrency::accelerator& srcDevice, Concurrency::accelerator& destDevice)
{
	...
	std::vector<Udt> srcCont(arrayExtent.size());
	std::fill(srcCont.begin(), srcCont.end(), Udt());
	array<Udt, 2> tempArray(arrayExtent, srcCont.begin(), srcDevice.default_view);
	array_view<Udt, 2> srcArrayView(tempArray);
	Log() << "Created array view on source device of " << srcArrayView.extent << std::endl;
	
	// Create destination array view
	array<Udt, 2> destDataArray(arrayExtent, destDevice.default_view);
	array_view<Udt, 2> destArrayView(destDataArray);
	Log() << "Created array view on destination device of " << destArrayView.extent << std::endl;
...
bool CopyBetweenStdContAndArrayView(Concurrency::accelerator& destDevice)
{
	...
	// Create destination array view
	array<Udt, 2> destDataArray(arrayExtent, srcCont.begin(), destDevice.default_view);
	array_view<Udt, 2> destArrayView(destDataArray);
	Log() << "Created array view on destination device of " << destArrayView.extent << std::endl;
