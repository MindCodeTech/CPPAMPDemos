...
runall_result test_d3d_device(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, accelerator_view av)
{
    ...
    { 
        Log(LogType::Info) << "Test on device :" << av.accelerator.description << std::endl;
...
runall_result test2()
{
    ...
    {
        accelerator_view rv = accelerator(accelerator::default_accelerator).default_view;
        ...
        result = test_d3d_device( pDevice, pContext, accelerator(accelerator::default_accelerator).default_view );
    }
