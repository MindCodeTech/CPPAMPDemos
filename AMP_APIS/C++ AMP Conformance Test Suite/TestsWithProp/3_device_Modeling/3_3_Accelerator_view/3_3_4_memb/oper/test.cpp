...
runall_result test_main()
{
    ...
    accelerator_view av = acc.default_view;
    ...
    accelerator_view av_cpu = acc_cpu.default_view;
    ...
    result &= REPORT_RESULT(av_assign_cpu == acc_cpu.default_view);
    result &= REPORT_RESULT(is_accelerator_view_equal(acc_cpu.default_view, av_assign_cpu));
    result &= REPORT_RESULT(is_accelerator_view_operable(av_assign_cpu));

    // verify == operator
    result &= REPORT_RESULT((av_assign_cpu == acc_cpu.default_view) == true);
    result &= REPORT_RESULT((av_assign_cpu == av_copy) == false);

    // verify != operator
    result &= REPORT_RESULT((av_assign_cpu != av_copy) == true);
    result &= REPORT_RESULT((av_assign_cpu != acc_cpu.default_view) == false);
