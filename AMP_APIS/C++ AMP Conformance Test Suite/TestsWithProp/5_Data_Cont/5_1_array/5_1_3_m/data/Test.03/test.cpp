...
bool test_feature()
{
    ...
    {
        array<_type, _rank> src(e1, data.begin(), data.end(), device.default_view);
        ...
        for(unsigned int i = 0; i < src.extent.size(); i++)
        {
    ...
    {
        const array<_type, _rank> src(e1, data.begin(), data.end(), device.default_view);
