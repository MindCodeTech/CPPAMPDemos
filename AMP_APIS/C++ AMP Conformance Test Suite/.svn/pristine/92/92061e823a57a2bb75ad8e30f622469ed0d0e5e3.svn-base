...
runall_result test_main()
{
	...
	if(device1.supports_cpu_shared_memory)
	{
		device1.set_default_cpu_access_type(DEF_ACCESS_TYPE1);
	}
	
	if(device2.supports_cpu_shared_memory)
	{
		device2.set_default_cpu_access_type(DEF_ACCESS_TYPE2);
	}

    accelerator_view av1 = device1.default_view;
    accelerator_view av2 = device2.default_view;
    ...
    parallel_for_each(av4, eC_half, [=](index<2> idx) restrict(amp)
        {
            ...
            for(int i = 0; i < mA_view1.extent[1]; ++i)
            {
    ...
    parallel_for_each(av4, eC_half, [=](index<2> idx) restrict(amp)
        {
            ...
            for(int i = 0; i < mA_view2.extent[1]; ++i)
            {
                index<2> idxA(idx[0], i);
                index<2> idxB(i, idx[1]);
