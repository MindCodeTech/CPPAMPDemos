...
{
	if(arr.cpu_access_type != exp_access_type)
	{
		Log(LogType::Error) << "Wrong cpu_access_type." << std::endl;
		Log(LogType::Error) << "Expect: " << exp_access_type << " Actual: " << arr.cpu_access_type << std::endl;
		return false;
	}
	
	if(arr.cpu_access_type != access_type_none && arr.data() == NULL)
	{
