...
int main()
{
    ...
    parallel_for_each(accel.default_view, extent<1>(1), [av](index<1>) __GPU {
        av[index<2>(0, 0)] = 1;
        av[index<2>(2, 2)] = 19;
    });
    
    // use this to get the results
    std::vector<int> v_results(av.extent.size());
    array_view<int, 2> results(av.extent, v_results);
    parallel_for_each(accel.default_view, av.extent, [av, results](index<2> i) __GPU {
        results[i] = av[i];
    });
