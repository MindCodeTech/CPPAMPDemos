...
ByteAddressBuffer ParticleDensityRO : register( t1 );
...
static const uint4 Rainbow[5] = {
    uint4(20, 102, 204, 255), // red
    uint4(0, 0, 255, 255), // orange
    uint4(20, 20,	255, 255), // green
    uint4(0, 0, 163, 255), // teal
    uint4(0, 0, 51, 255), // blue
};

float4 VisualizeNumber(float n)
{
    return lerp( Rainbow[ floor(n * 4.0f) ] / 255.0f, Rainbow[ ceil(n * 4.0f) ] / 255.0f, frac(n * 4.0f) );
}
...
VSParticleOut ParticleVS(uint ID : SV_VertexID)
{
    ...
    Out.position.x = asfloat(ParticlesRO.Load(ID*4*4));
	Out.position.y = asfloat(ParticlesRO.Load((ID*4 + 1) * 4));
    Out.color = VisualizeNumber(asfloat(ParticleDensityRO.Load(ID * 4)), 1000.0f, 2000.0f);
    return Out;
}
