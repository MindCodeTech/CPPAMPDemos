...
m_fft_plan(3, m_av)
{
    ...
    memset(h0_data.data(), 0, height_map_size * sizeof(XMFLOAT2));
...
void ocean_simulator::init_height_map(ocean_parameter& params, vector<float_2> &out_h0, vector<float> &out_omega)
{
	...
	XMFLOAT2 wind_dir;
	XMVECTOR vwind_dir = XMLoadFloat2(&wind_dir);
	// XMVector2Normalize(&wind_dir, &params.wind_dir);
	XMStoreFloat2(&wind_dir, XMVector2Normalize(params.vwind_dir));
