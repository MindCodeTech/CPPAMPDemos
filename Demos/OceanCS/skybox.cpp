                                        ...
                                        ID3D11Texture2D* pCubeTexture, ID3D11ShaderResourceView* pCubeRV )
{
    ...
    // Create the shaders
    tstring filePath = GetFilePath::GetFilePath(_T("\\..\\Media\\Shaders\\skybox.hlsl"));
    ...
    V_RETURN( CompileShaderFromFile( (WCHAR *)filePath.c_str(), nullptr, "SkyboxVS", "vs_4_0", D3DCOMPILE_ENABLE_STRICTNESS, 0, &pBlobVS ) );
    V_RETURN( CompileShaderFromFile( (WCHAR *)filePath.c_str(), nullptr, "SkyboxPS", "ps_4_0", D3DCOMPILE_ENABLE_STRICTNESS, 0, &pBlobPS ) );
...
void CSkybox11::OnD3D11ResizedSwapChain( const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    pVertex[0].pos = MHVECTOR4( fLowW, fLowH, 1.0f, 1.0f );
	pVertex[1].pos = MHVECTOR4(fLowW, fHighH, 1.0f, 1.0f);
	pVertex[2].pos = MHVECTOR4(fHighW, fLowH, 1.0f, 1.0f);
	pVertex[3].pos = MHVECTOR4(fHighW, fHighH, 1.0f, 1.0f);
...
void CSkybox11::D3D11Render(const MHMATRIX& pmWorldViewProj, ID3D11DeviceContext* pd3dImmediateContext)
{
    HRESULT hr;
    ...
    // XMMatrixInverse( &pVSPerObject->m_WorldViewProj, nullptr, mWorldViewProj );
	MHMATRIX m = MHMATRIX::Inverse(pmWorldViewProj);
