...
namespace ptm
{
	...
	template<typename T> Pattern_matcher<T>::Pattern_matcher(void)
	{
		...
		dist_funcs.emplace(std::begin(dist_funcs) + Geo, clc_geo_dst, [ ](const Vec_t& dist) -> T { 
			static const T radius(static_cast<T>(6378.388)); // Radius of idealized earth sphere.
	...
	{		
		...
		// bparadie, 2012-09-10: ugly non-const cast (shame on me!)
		std::vector<T>& coords = const_cast<pbl::Tsp_container&>(params).pbl_coords;

		coords.reserve(params.problem_dim*2);
		...
		while (data_it != end_it) {
			...
			// bparadie, 2012-09-10: pbl_coords holds the coordinates
