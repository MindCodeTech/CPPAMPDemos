...
{
	for(int i = 0; i < av.extent[1]; i++)
	{
		for(int j = 0; j < av.extent[0]; j++)
		{
			if(vec[i * av.extent[1] + j] != av(i,j))
			{
				Log(LogType::Error) << compose_incorrect_element_message(index<2>(i,j), vec[i * av.extent[1] + j], av(i,j)) << std::endl;
				return false;
			}
		}
	}
...
bool compare(const vector<T>& vec, const array_view<T, 3>& av)
{
	for(int i = 0; i < av.extent[0]; i++)
	{
		for(int j = 0; j < av.extent[1]; j++)
		{
			for(int k = 0; k < av.extent[2]; k++)
			{
				if(vec[i * av.extent[1] * av.extent[2] + j * av.extent[2] + k] != av(i,j,k))
				{
					Log(LogType::Error) << compose_incorrect_element_message(index<3>(i,j,k), vec[i * av.extent[1] * av.extent[2] + j * av.extent[2] + k], av(i,j,k)) << std::endl;
					return false;
				}
			}
		}
...
bool verify_extent(concurrency::array_view<TValue, Rank> actual, concurrency::extent<Rank> expected)
{
	...
	if(actual.extent != expected)
	{
		Log(LogType::Error) << "Extent doesn't match expected "
			<< "Expected: " << expected << " Actual: " << actual.extent << std::endl;
		return false;
	}
