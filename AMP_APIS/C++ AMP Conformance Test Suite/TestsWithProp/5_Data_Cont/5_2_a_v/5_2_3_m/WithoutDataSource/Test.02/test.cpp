...
bool Test2()
{
    ...
    parallel_for_each(arrViewSum.extent, [=](const index<2> &idx) restrict(amp) {
        arrViewSum[idx] = arrViewA[idx] + arrViewB[idx];
        arrViewDiff[idx] = arrViewA[idx] - arrViewB[idx];
...
bool Test3(const accelerator_view &av)
{
    ...
    parallel_for_each(av, tempArr.extent, [=, &tempArr](const index<2> &idx) restrict(amp) {
...
bool Test4()
{
    ...
    parallel_for_each(arrViewSum.extent, [=](const index<1> &idx) restrict(amp) {
        arrViewSum[idx] = arrViewA[idx] + arrViewB[idx];
        arrViewDiff[idx] = arrViewA[idx] - arrViewB[idx];
        arrViewMul[idx] = arrViewA[idx] * arrViewB[idx];
        arrViewDiv[idx] = arrViewA[idx] / (arrViewB[idx] + 1);
...
bool Test5(const accelerator_view &av)
{
    ...
    parallel_for_each(av, tempArr.extent, [=, &tempArr](const index<1> &idx) restrict(amp) {
        tempArr[idx] = arrViewA[idx] + arrViewB[idx];
    });
...
{
    outStream << "(" << val.x << ", " << val.y << ", " << val.z << ", " << val.w << ")";
    return outStream;
}
...
runall_result Test9()
{
    ...
    {
        if (allAccls[i].device_path == accelerator::cpu_accelerator) {
            continue;
        }

        if (allAccls[i].supports_limited_double_precision) {
            foundLimitedDoubleSupportAccl = true;
            av = allAccls[i].default_view;
            break;
        }
    }
...
bool Test13()
{
    ...
    array_view<int, 2> arrViewDiff(M, N);
    parallel_for_each(arrViewDiff.extent, [=](const index<2> &idx) restrict(amp) {
        arrViewDiff[idx] = 0;
    });
...
bool Test15()
{
    ...
    parallel_for_each(arrA.extent, [&](const index<1> &idx) restrict(amp) {
        arrA[idx] = idx[0];
    });
    ...
    std::vector<int> vec(arrB.extent.size(), 0);
