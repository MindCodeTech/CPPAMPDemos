...
runall_result test_main()
{
    ...
    std::for_each(accs.begin(), accs.end(),
        [&](accelerator& acc)
    {
        Log(LogType::Info) << "For device : " << acc.description << std::endl;

        // default accelerator view
        Log(LogType::Info) << "Default view : " << std::endl;
        result &= REPORT_RESULT(acc.default_view.queuing_mode == queuing_mode_automatic);
        result &= REPORT_RESULT(acc.default_view.queuing_mode != queuing_mode_immediate);
        ...
        result &= REPORT_RESULT(av_imm.queuing_mode == queuing_mode_immediate);
        result &= REPORT_RESULT(av_imm.queuing_mode != queuing_mode_automatic);
        ...
        result &= REPORT_RESULT(av_auto.queuing_mode == queuing_mode_automatic);
        result &= REPORT_RESULT(av_auto.queuing_mode != queuing_mode_immediate);
    }
    );
