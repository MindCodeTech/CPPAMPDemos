...
{	
	...
	if(gpuDevice.supports_cpu_shared_memory)
	{
		gpuDevice.set_default_cpu_access_type(ACCESS_TYPE);
	}
	
	std::vector<int> data(DATA_SIZE, 2);
	array<int, 1> dataArray(DATA_SIZE, data.begin(), gpuDevice.default_view);
	...
	parallel_for_each(arrayView1.extent, [=, &dataArray](index<1> idx) restrict(amp) {
